...
| [OPRS.md](OPRS.md) | **Open Packet Reporting System**: the complete protocol specification, an APRS replacement needing no licence. One syntax (`key:value` fields separated by spaces), callsigns of any length, and packet types for messages, blog posts, observations, tracks, passages, events, offers and needs, radio channels, calls for help, warnings, notices and callsign challenges. Identifiers are derived rather than transmitted, so the same message arriving by two routes is recognised once; a message can be carried toward a place by whoever is going that way, and answered with a signed receipt naming the route it took. Every measurement carries its unit, prices carry their currency, and `cw:` warns what a packet contains before it renders. Section 27 is a printable cheat sheet and section 28 states what the shipping code does not yet do. |
| [spectrum.md](spectrum.md) | **Where OPRS packets go on each bearer**, and why the answer is not one number: the regional LoRa bands with a proposed calling frequency for each, HaLow and 2.4 GHz WiFi allocations, CB and PMR446 and their incompatible cousins, the amateur bands a licensed operator may bridge on, and a four-step plan for connectionless WiFi that starts with WiFi Aware because monitor mode needs root on almost every phone. Status: proposal, none of it implemented. |

...
| [OPRS.md](OPRS.md) | **Open Packet Reporting System**: the complete protocol specification, an APRS replacement needing no licence. One syntax (`key:value` fields separated by spaces), callsigns of any length, and packet types for messages, blog posts, observations, tracks, passages, events, offers and needs, calls for help, warnings, notices and callsign challenges. Identifiers are derived rather than transmitted; one vocabulary asks and answers; relaying records its own path; every measurement carries its unit; prices carry their currency; and `cw:` warns what a packet contains before it renders. Section 26 is a printable cheat sheet and section 27 states what the shipping code does not yet do. |

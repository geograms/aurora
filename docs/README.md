...
| [REPR.md](REPR.md) | **Reticulum Packet Radio** — the APRS-shaped network for people without an amateur licence: what each APRS concept maps to here (callsign, digipeater, iGate, bulletin, beacon, directory), what is already built, and what is still missing (LoRa on the data path, position as a real object, node coverage). |

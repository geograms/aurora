...
| [XPRS.md](XPRS.md) | **eXtended Packet Radio System**: the complete protocol specification, an APRS replacement needing no licence. One syntax (`key:value` fields separated by spaces), callsigns of any length, and 21 packet types covering messages, blog posts, observations, tracks, passages, events, offers and needs, radio channels, mailboxes, calls for help, warnings, notices and callsign challenges. Identifiers are derived from the packet rather than transmitted, so a message arriving by two routes is recognised once. Packets are signed by default; a message can be carried toward a place by whoever is going that way and answered with a signed receipt naming the route; `scope:` keeps community traffic off the internet; `lang:` names the language and `nick:` the operator. Every measurement carries its unit, prices their currency, and `cw:` warns what a packet contains before it renders. Section 27 is a printable cheat sheet and section 28 states what the shipping code does not yet do. |

...
| [OPRS.md](OPRS.md) | **Open Packet Reporting System**: the complete protocol specification. Frame and addressing, callsigns, messages, threads, reactions, receipts, parcels, attachments, signing and encryption with the per-band rules, and observations covering coordinates, movement, weather and telemetry in one token vocabulary. Includes worked examples and exchanges. |

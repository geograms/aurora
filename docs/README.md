...
| [XPRS.md](XPRS.md) | **eXtended Packet Radio System**: the complete protocol specification, an APRS replacement needing no licence. One syntax (`key:value` fields separated by spaces), callsigns of any length, and packet types for messages, blog posts, observations, tracks, passages, events, offers and needs, radio channels, calls for help, warnings, notices and callsign challenges. Identifiers are derived rather than transmitted, so the same message arriving by two routes is recognised once; a message can be carried toward a place by whoever is going that way, and answered with a signed receipt naming the route it took. Every measurement carries its unit, prices carry their currency, and `cw:` warns what a packet contains before it renders. Section 27 is a printable cheat sheet and section 28 states what the shipping code does not yet do. |

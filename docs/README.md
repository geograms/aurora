...
| [OPRS.md](OPRS.md) | **Open Packet Reporting System**: the APRS-shaped network for operators without an amateur licence. Maps each APRS concept to its OPRS equivalent (callsign, digipeater, iGate, bulletin, beacon, directory) and marks each BUILT, PARTIAL or NOT BUILT. |
| [oprs-data.md](oprs-data.md) | **The OPRS wire format specification**: one 250-byte frame for every transport, covering messages, threads, reactions, receipts, parcels, signing and encryption (with the per-band rules), plus position, movement, weather and telemetry in one token vocabulary using SI units and a mandatory time field. Includes worked exchanges. |

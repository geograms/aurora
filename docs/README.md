...
| [OPRS.md](OPRS.md) | **Open Packet Reporting System** — the APRS-shaped network for people without an amateur licence: what each APRS concept maps to here (callsign, digipeater, iGate, bulletin, beacon, directory), what is already built, and what is still missing (LoRa on the data path, position as a real object, node coverage). |
| [oprs-data.md](oprs-data.md) | **The OPRS wire format** — one 250-byte frame for every radio: messages, threads, reactions, receipts, parcels, signing and encryption (and where each is legal), plus position, movement, weather and telemetry in one token vocabulary with SI units and a mandatory time field. |

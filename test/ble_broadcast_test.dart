...
// matching what the scan API hands us).
Uint8List primary(int msgId, int idx, int total, List<int> payload,
        {bool hasCont = false}) =>
    Uint8List.fromList(
        [kBleMarker, kBleBcastPrimary, msgId, idx, total, hasCont ? 1 : 0, ...payload]);
Uint8List cont(int msgId, int idx, List<int> payload) =>
    Uint8List.fromList([kBleMarker, kBleBcastCont, msgId, idx, ...payload]);

// Split a payload into chunks the way the sender would, then return the advert
// entries (primary [+cont]) in transmission order.
List<Uint8List> chunkify(int msgId, List<int> payload,
  ...
  for (var idx = 0; idx < total; idx++) {
    ...
    out.add(primary(msgId, idx, total, pPart, hasCont: cPart.isNotEmpty));
    if (cPart.isNotEmpty) out.add(cont(msgId, idx, cPart));
  }
  return out;
}
...
void main() {
  group('BleBroadcastReassembler', () {
    test('single-chunk message (ADV-only) reassembles', () {
      ...
      final entries = chunkify(7, payload);
    ...
    test('multi-chunk ~300B message reassembles (primary + continuation)', () {
      ...
      final entries = chunkify(9, payload);
    ...
    test('out-of-order and duplicated chunks still reassemble once', () {
      ...
      final entries = chunkify(5, payload);
    ...
    test('dedup: replaying a completed message does not re-deliver', () {
      ...
      final entries = chunkify(3, payload);
      var deliveries = 0;
    ...
    test('same msgId from different sources are independent', () {
      ...
      final ea = chunkify(1, pa);
      final eb = chunkify(1, pb); // same msgId, different source
      Uint8List? gotA, gotB;
    ...
    test('continuation arriving before its primary is tolerated', () {
      ...
      final entries = chunkify(2, payload);
    ...
    test('ADV-only sender format (Flutter, no continuation) round-trips', () {
      ...
      for (var idx = 0; idx < total; idx++) {
        ...
        got = r.ingest('peerA',
                primary(99, idx, total, payload.sublist(off, end))) ??
            got;
      }
      expect(got, Uint8List.fromList(payload));
    });

...
// (native/src/api_wasmi.rs:634). Declaring the first parameter 64-bit here
// happens to work on AAPCS64 — each integral argument gets its own register,
// so Dart reads w0/x1 and Rust wrote w0/x1 — but it is fatal on AAPCS32
// (armeabi-v7a): a 64-bit argument must occupy an even-aligned CORE REGISTER
// PAIR, so this trampoline expected the id in r0:r1 and the args pointer in
// r2, while Rust passed that pointer in r1. Dart then dereferenced a stale
// register as a Dart_CObject* and threw
//   "Can't read invalid data type -509558769"
// (0xE1A0C00F = the ARM32 encoding of `mov ip, pc`, the first word of a PLT
// veneer — i.e. it was reading executable code), after which the process died
// with SIGSEGV. Every wasm->host call was broken on 32-bit ARM; a phone in the
// same build was fine. Matching Rust's u32 fixes 32-bit and changes nothing on
// 64-bit.
typedef GlobalWasmFunction = ffi.Pointer<wire_list_wasm_val> Function(
  ffi.Uint32 functionId,
...
class _References {
  ...
  static ffi.Pointer<wire_list_wasm_val> _globalWasmFunction(
    ...
    ffi.Pointer<wire_list_wasm_val> pointer;
    ...
    try {
      ...
      // (bridge_generated.rs:4085), dereferencing address 0. So ANY host
      // function error became a native SIGSEGV that no Dart try/catch could
      // contain. An empty result list instead reaches Rust as an empty
      // Vec<WasmVal>, which surfaces as an ordinary wasm trap/arity error.
      try {
        // ignore: invalid_use_of_protected_member
        pointer = platform.api2wire_list_wasm_val(const []);
      } catch (_) {
        rethrow; // nothing safe left to hand back
      }
    }
    return pointer;
  }
